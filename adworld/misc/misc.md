...
解压附件后，得到一个高分辨率的图片。
直接托到`Stegsolve`分析，发现二维码。扫码会得到一串摩斯密码`...--.----...--..`

![big.png](./misc.assest/figure-qr.png)

在线解码，发现无法识别，再观察一下，摩斯编码的长度为3或者4。
结合题目信息，重新对得到的摩斯密码进行4433分组`...- -.-- --. ..- -..`
得到内容`VYGUD`

提交，错误。

再次搜索资料，发现`VYGUD`在摩斯码里，是`verygood`的缩写。
